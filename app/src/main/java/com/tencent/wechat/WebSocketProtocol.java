...
public final class WebSocketProtocol {
    ...
    public static String closeCodeExceptionMessage(int i) {
        ...
        if ((i < 1004 || i > CLOSE_ABNORMAL_TERMINATION) && (i < 1012 || i > 2999)) {
            return null;
        }
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public static void toggleMask(byte[] bArr, long j2, byte[] bArr2, long j3) {
        int length = bArr2.length;
        int i = OPCODE_CONTINUATION;
        while (i < j2) {
            bArr[i] = (byte) (bArr2[(int) (j3 % length)] ^ bArr[i]);
            i += OPCODE_TEXT;
            j3++;
        }
    }
